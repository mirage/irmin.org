...
const questions = {
    "git": [
        "Do you need to be compatible with Git?",
        ["Yes", "No"]
    ],
    "storage": [
...
class Wizard extends React.Component {
        ...
        const possibleOutcomes = Object.keys(outcomes).filter(k => {
            ...
            return this.state.answers.every((choice, index) => {
                ...
                if (Array.isArray(outcome[index])) {
                    return outcome[index][0] === choice[0] && outcome[index][1] === choice[1];
                } else {
    ...
    render() {
        ...
        if (outcome !== null) {
            var message = <p>You have reached outcome {outcome}.</p>;
        } else {
            var message = <p>You haven't reached an outcome yet.</p>;
        }

        return (
            <div className="wizard">
                <ol>
                    {questions.map(([q, ai], i) => <Question key={q} name={q}
                        questionIndex={i} currentAnswerIndex={ai}
